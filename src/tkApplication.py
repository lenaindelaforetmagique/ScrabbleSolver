...
from tkCanvasObject import CanvasObject

from random import *
from ScrabbleGame import ScrabbleGame, _readLines, _coord
...
class Application(tk.Tk):
    ...
    def pickLetters(self, letters):
        ...
        if not action[0]:
            ...
            self.SelectTurn(self.turnCount)
            self.canvas.Refresh()
    ...
    class LeftRegion(tk.Canvas):
        ...
        def RefreshRack(self):
            for letter in self.LettersRack:
                letter.turnNumber = 1000 # big enough
                letter.active = False
                letter.refresh()
            ...
            for i, letterG in enumerate(self.game.rack):
                ...
                while self.LettersRemainder[j].letter != letterG:
                    j += 1
                self.LettersRack.append(self.LettersRemainder.pop(j))
                self.LettersRack[-1].turnNumber = self.turnCount #self.turnHighlight
                self.LettersRack[-1].active = True
                if self.LettersRack[-1].letter.value == 0:  # if face has been modified for any reason, re-set to '?'
        ...
        def NewWord(self):
            for i, line in enumerate(self.game.grid):
                for j, square in enumerate(line):
                    if self.LettersGrid[i][j] is None and square.letter != None:
                        letter = square.letter
                        # new letter to find in rack
                        k = 0
                        while self.LettersRack[k].letter != letter:
                            k += 1
            ...
            for i, line in enumerate(self.LettersGrid):
                for j, letter in enumerate(line):
                    if letter is None:
                        ...
                        if letter.turnNumber <= self.turnHighlight:
                            letter.active = True
                        else:
                            letter.active = False
                        letter.refresh()
...
#                     if letter.turnNumber <= self.game.turnHighlight:
#                         letter.active = True
#                     else:
#                         letter.active = False
#
            ...
            def draw(self):
                ...
                if self.letter.value > 0:
                    ...
                    self.tkItems.append(self.parent.create_text(self.x+7,
                                                         self.y+4,
                                                         text = self.letter.face,
                                                         fill="#342D27",
                                                         font=style["letter"],
                                                         anchor=tk.NW))
                    self.tkItems.append(self.parent.create_text(self.x+self.tileSize-4,
                                                         self.y+self.tileSize-3,
                                                         text = self.letter.value,
                                                         fill="#342D27",
                                                         font=style["letter2"],
                                                         anchor=tk.SE))
                else:
                    # Joker
                    self.tkItems.append(self.parent.create_text(self.x+13,
                                                         self.y+1,
                                                         text = self.letter.face,
                                                         fill="#FF00FF",
                                                         font=style["joker"],
